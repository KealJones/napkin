...
type ConceptUnit = {
  ...
  realizations: Expr[];
  updatedAt?: string;
  usage?: { events: number; lastUsedAt?: string };
};
type Activity = {
  id: string;
  meaning: string | null;
...
function argsOf(expression: Expr | undefined, name: string): Expr | undefined {
  if (!expression || typeof expression !== "object" || !("apply" in expression))
    return undefined;
...
function headOf(expression: Expr | undefined): string {
  return expression && typeof expression === "object" && "apply" in expression
...
function prettyExpression(expression: Expr, depth = 0): string {
  ...
  const flat = `${expression.apply.head}(${args.join(", ")})`;
  if (flat.length < 86 && !flat.includes("\n")) return flat;
  if (args.length === 0) return `${expression.apply.head}()`;
  const indent = "  ".repeat(depth);
  return `${expression.apply.head}(\n${args
...
function activityFromConversation(unit: ConversationUnit): ChatMessage[] {
  ...
  for (const relation of unit.relations ?? []) {
    ...
    if (head === "Message") {
      ...
      const meaning =
        relation && typeof relation === "object" && "apply" in relation
          ? relation.apply.args[0]?.value
          : undefined;
      latestUser.activity.meaning = meaning ? formatExpression(meaning) : null;
  ...
  async function saveConcept(
    ...
    try {
      const payload = {
        ...
        gloss,
        relations: JSON.parse(relations) as Expr[],
        realizations: JSON.parse(realizations) as Expr[],
      };
    ...
    <div className="app">
      <aside className="sidebar">
        ...
        <div className="side-bottom">
          ...
          Local runtime
          <br />
          <span className="side-indent">SQLite · Ollama</span>
        </div>
      </aside>
      <main className="main">
                ...
                <div className="concept-items">
                  {visibleConcepts.map((unit) => (
                    ...
                    >
                      ...
                      <small>
                        {unit.gloss} · {unit.usage?.events ?? 0} traced calls
                      </small>
                    </button>
                  ))}
...
function ConceptEditor({
  ...
  const expressionPreview = useMemo(() => {
    try {
      const relationValues: unknown = JSON.parse(relations);
      const realizationValues: unknown = JSON.parse(realizations);
      if (
        !Array.isArray(relationValues) ||
        !relationValues.every(isExpr) ||
        !Array.isArray(realizationValues) ||
        !realizationValues.every(isExpr)
      ) {
        return "Relations and realizations must be JSON arrays of Concept expressions.";
      }
      return prettyExpression(
        application("Concept", [
          ...
          },
          {
            ...
            value: application(
              "Realizations",
              realizationValues.map((value) => ({ value })),
            ),
          },
        ]),
      );
    } catch {
      return "Fix the relations and realizations JSON to preview this Concept.";
    }
  }, [concept?.identity, gloss, relations, realizations]);
  useEffect(() => {
    setGloss(concept?.gloss ?? "");
    setRelations(JSON.stringify(concept?.relations ?? [], null, 2));
    ...
    <article className="concept-editor">
      <div className="editor-heading">
        <div>
          ...
          <small className="muted">
            {concept.usage?.events ?? 0} traced calls · last used{" "}
            {concept.usage?.lastUsedAt
              ? new Date(concept.usage.lastUsedAt).toLocaleString()
              : "never"}
          </small>
        </div>
        <div className="editor-actions">
          ...
          <button
            className="primary"
            onClick={() =>
              void onSave(concept, gloss, relations, realizations).then(
                setSaved,
              )
            }
          >
      ...
      <div className="field">
        ...
        />
        <small>
          Searchable description; conceptual meaning and behavior live in the
