...
2. WordNet synonymy is per sense and Cnocept names are per word. Importing `SynonymOf`
...
- **FrameNet**: ~1,200 frames (1,222 in release 1.7), ~13,500 lexical units, 200,751 annotated sentences; frames are cross-linguistically stable (Buyer/Seller/Goods). Multilingual FrameNets exist (Spanish, German, Japanese, Korean, Portuguese, Chinese, Swedish). Good model for Cnocept's Frame layer.
...
- **Grammatical Framework**: abstract syntax as interlingua; the Resource Grammar Library covers 40+ languages from one shared abstract syntax; the exact "English-named abstract layer with per-language concrete syntaxes" pattern Cnocept faces. Ranta et al. (2020) note the abstract syntax "offers a unified view on … Universal Dependencies, WordNets, FrameNets, Construction Grammars, and Abstract Meaning Representations."
...
- Normalize both into Cnocept's ~30–40 canonical relation types. Context-scoped synonyms (the `GoesInto` vs `DividedBy` operand-order case) should be modeled as `SynonymOf` with a context qualifier, mirroring ConceptNet's `HasContext` relation and FrameNet's frame-scoping.
...
- NGSL and ConceptNet are share-alike (CC BY-SA); if Cnocept's seed is redistributed, derived data inherits share-alike. Keep these in a separable layer so the CC0/CC BY core stays unencumbered.
...
- "Coverage" figures are in word families and drawn from written/spoken corpora (Cambridge English Corpus, BNC, Wellington) that may not match Cnocept's actual traffic; validate against real utterances before freezing tier sizes.
