...
const BASE_NOPLACEHOLDER = BASE.replace(
`Fact(x)  Do(x)  Tell(to, content)  Aside("verbatim text")
Whether(proposition)   a yes/no question
Me()  You()  Today()  Date(when)  Multiply(a,b)  Count(needle, haystack)`,
`Fact(...)        the user asserting something
Do(...)          the user requesting an action
Tell(...)        delivery, as in Tell(Me(), Answer())
Aside("...")     a side remark, verbatim
Whether(...)     a yes/no question
Me()  You()  Today()  Date(...)  Multiply(...)  Count(...)`);

const V = {
  B1_rules:   [BASE, RULES, OUT].join("\n\n"),
  B3_noplace: [BASE_NOPLACEHOLDER, RULES, OUT].join("\n\n"),
};
