 ...
 * yet: whatever kind it seeds only has to declare `Ends()` on itself for this to see it.
 */
function isOpen(store: ConceptStore, identity: string): boolean {
  ...
  const definesEnding = (id: string) => (store.get(id)?.relations ?? []).some((r) => isCall(r.claim) && r.claim.head === "Ends");
