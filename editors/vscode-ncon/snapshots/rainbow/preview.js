...
function paint(text) {
  ...
  for (const m of flat) {
    ...
    /** @type {string} */
    let style;
    if (m.kind === "head") style = `color:${depth}`;
      ...
      const p = /** @type {Record<string, { color: string, fontStyle?: string }>} */ (PALETTE)[m.kind];
...
<div class="legend">nesting depth: ${RAINBOW.map((c, i) => `<span style="background:${c}" title="depth ${i}"></span>`).join("")} then it cycles</div>
${sections.join("\n")}
`);
